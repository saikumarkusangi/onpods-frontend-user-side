  ...
  try {
    ...
    if (existingIndex != -1) {
      ...
      if (page == 1 && existingQuotes.page != 1) {
        final Set<String> existingIds = existingQuotes.datas.map((e) => e.id).toSet();
        final newData = data[0].datas.where((newItem) => !existingIds.contains(newItem.id)).toList();
        existingQuotes.datas.insertAll(0, newData); // Add new data before existing data.
